...
/**
 * Quaternion multiply (Hamilton product). MuJoCo convention: [w, x, y, z].
 * @param {number[]} q1 - First quaternion.
 * @param {number[]} q2 - Second quaternion.
 * @returns {number[]} Product quaternion [w, x, y, z].
 */
 ...
 * @returns {number[]} Conjugate [w, -x, -y, -z].
 */
 ...
 * Ported from mjlab math.py:axis_angle_from_quat.
 * @param {number[]} q - Quaternion [w, x, y, z].
 * @returns {number[]} Axis-angle vector [ax, ay, az].
 */
export function axisAngleFromQuat(q) {
  // Ensure w >= 0 (pick the shorter path).
  let [w, x, y, z] = q;
  if (w < 0) { w = -w; x = -x; y = -y; z = -z; }

  const mag = Math.sqrt(x * x + y * y + z * z);
  const halfAngle = Math.atan2(mag, w);
  const angle = 2.0 * halfAngle;

  // Taylor approximation near zero: sin(θ/2)/θ ≈ 0.5 - θ²/48
  const EPS = 1e-6;
  const sinHalfOverAngle =
    Math.abs(angle) > EPS
      ? Math.sin(halfAngle) / angle
      : 0.5 - (angle * angle) / 48;
...
/**
 * Quaternion box-minus: axis-angle difference q1 ⊖ q2.
 * Returns axisAngle(q1 * conj(q2)).
 * @param {number[]} q1 - Target quaternion [w, x, y, z].
 * @param {number[]} q2 - Current quaternion [w, x, y, z].
 * @returns {number[]} Axis-angle error [ax, ay, az].
 */
export function quatBoxMinus(q1, q2) {
  return axisAngleFromQuat(quatMultiply(q1, quatConjugate(q2)));
}

/**
 * Quaternion from intrinsic XYZ Euler angles (roll, pitch, yaw).
