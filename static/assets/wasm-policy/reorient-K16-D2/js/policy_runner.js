...
export class PolicyRunner {
  ...
  constructor(viewer, config) {
    ...
    this._running = false;
    this.onStep = null; // callback(qpos, lastAction) per step
  ...
  start() {
    ...
    const animate = async () => {
      ...
      if (this.onStep) {
        ...
        const qpos = Array.from(data.qpos.slice(cfg.qpos_joint_offset, cfg.qpos_joint_offset + cfg.num_joints));
        // Extract position targets (ctrl values mapped back to joint order)
        const targets = new Array(cfg.num_joints).fill(0);
        for (let i = 0; i < Math.min(nAct, cfg.num_joints); i++) {
          const ctrlIdx = ctrlMap ? ctrlMap[i] : i;
          targets[i] = data.ctrl[ctrlIdx];
        }
        this.onStep(qpos, targets);
      }
  ...
  _checkReorientSuccess(data) {
    ...
    // Sustained success → resample goal (+ reset timer).
    ...
    if (this._consecutiveAtGoal >= sustainedSteps) {
      ...
      this._resampleReorientCommand(!!cfg.resample_goal_only);
      this._resetReorientTimer();
    }
  }
  ...
  _colorCubeBodies() {
    ...
    for (const cmd of Object.values(cmdCfg)) {
      if (cmd.type !== "reorientation") continue;
