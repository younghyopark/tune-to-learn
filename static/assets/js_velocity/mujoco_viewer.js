...
export class MuJoCoViewer {
  ...
  updateDragForces() {
    ...
    // Spring force: F = mass * scale * displacement.
    const disp = this._dragCursorWorld.clone().sub(this._dragPointWorld);
    const len = disp.length();
    if (len < 0.001) return;

    const mass = this.model.body_mass[this._dragBodyId];
    const forceScale = mass * 250;
    const forceVec = _toMujocoPos(disp.clone().multiplyScalar(forceScale));
  ...
  async _loadScene() {
    ...
    const meshFiles = this._parseMeshFiles(xmlText);
    ...
    const vfsMeshDir = "/working/" + meshDir;
    this._ensureDir(vfsMeshDir);

    this.onStatus(`Loading ${meshFiles.length} mesh files...`);
    await Promise.all(
      meshFiles.map(async (filename) => {
        const url = baseURL + meshDir + filename;
        const response = await fetch(url);
        if (!response.ok) {
          console.warn(`Failed to fetch mesh: ${url}`);
          return;
        }
        const buffer = await response.arrayBuffer();
        const fullPath = vfsMeshDir + filename;
  ...
  _parseMeshFiles(xmlText) {
    const files = [];
    const regex = /file\s*=\s*"([^"]+\.[sS][tT][lL])"/g;
    let match;
  ...
  _onPointerDown(e) {
    ...
    for (const hit of hits) {
      ...
      // Skip world body (0) and fixed base (1).
